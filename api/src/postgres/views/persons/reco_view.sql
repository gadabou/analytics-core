CREATE MATERIALIZED VIEW IF NOT EXISTS reco_view AS 
    ...
    FROM couchdb  
        LEFT JOIN village_secteur_view v ON (doc->'parent'->>'_id')::TEXT = v.id 
        LEFT JOIN district_quartier_view d ON (doc->'parent'->'parent'->>'_id')::TEXT = d.id 
        LEFT JOIN hospital_view h ON (doc->'parent'->'parent'->'parent'->>'_id')::TEXT = h.id 
        LEFT JOIN commune_view cm ON (doc->'parent'->'parent'->'parent'->'parent'->>'_id')::TEXT = cm.id 
        LEFT JOIN prefecture_view p ON (doc->'parent'->'parent'->'parent'->'parent'->'parent'->>'_id')::TEXT = p.id 
        LEFT JOIN region_view r ON (doc->'parent'->'parent'->'parent'->'parent'->'parent'->'parent'->>'_id')::TEXT = r.id 
        LEFT JOIN country_view c ON (doc->'parent'->'parent'->'parent'->'parent'->'parent'->'parent'->'parent'->>'_id')::TEXT = c.id 
