CREATE MATERIALIZED VIEW IF NOT EXISTS hospital_manager_view AS 
    ...
    FROM couchdb 
        LEFT JOIN hospital_view h ON (doc->'parent'->>'_id')::TEXT = h.id 
        LEFT JOIN commune_view cm ON (doc->'parent'->'parent'->>'_id')::TEXT = cm.id 
        LEFT JOIN prefecture_view p ON (doc->'parent'->'parent'->'parent'->>'_id')::TEXT = p.id 
        LEFT JOIN region_view r ON (doc->'parent'->'parent'->'parent'->'parent'->>'_id')::TEXT = r.id 
        LEFT JOIN country_view c ON (doc->'parent'->'parent'->'parent'->'parent'->'parent'->>'_id')::TEXT = c.id 
